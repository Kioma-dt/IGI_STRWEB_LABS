...
class UserInput:
    ...
    def get_int_list(prompt : str, stop_number : int = 0) -> list[int]:
        ...
        x = UserInput.get_int("Input Integer Value: ")
    ...
    def get_float_list(prompt : str, list_size : int) -> list[float]:
        ...
        for x in range(list_size):
            x = UserInput.get_float("Input Float Value: ")
            lst.append(x)
...
class ProductMixin:
    """Mixin To Add Working With Products"""


    @staticmethod
    @repeat_input
    def get_product_name(prompt: str, products: list[Product]) -> str:
        ...
        ProductAnalyzer.get_product_info(products, product_name)

        return product_name
    

class ProductUserInput(UserInput, ProductMixin):
    pass