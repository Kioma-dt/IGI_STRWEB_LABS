...
class StoreHomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        banners = list(
            Banner.objects.filter(is_deleted=False, is_active=True).order_by(
                "sort_order",
                "title",
            )
        )
        ctx["banners"] = banners
        ctx["banner"] = random.choice(banners) if banners else None
        ...
        ctx["partners"] = (
            Partner.objects.filter(is_deleted=False, is_active=True)
...
class StoreAboutView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        company = (
            CompanyInfo.objects.filter(is_deleted=False, is_current=True)
            .prefetch_related("history_entries")
            .first()
        )
        ctx["company"] = company
        if company:
            ctx["history_entries"] = company.history_entries.filter(
                is_deleted=False,
            ).order_by("year")
        else:
            ctx["history_entries"] = []
...
class StoreContactsView(FormView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        contacts_qs = Contact.objects.filter(is_deleted=False).order_by(
            "-is_primary",
            "type",
        )
        ctx["employees"] = contacts_qs.filter(type=Contact.ContactType.EMPLOYEE)
        ctx["contacts"] = contacts_qs.exclude(type=Contact.ContactType.EMPLOYEE)
        ctx["company"] = (
            CompanyInfo.objects.filter(is_deleted=False, is_current=True).first()
        )
        return ctx
    ...
    def post(self, request, *args, **kwargs):
        ...
        for key in request.POST:
            if key.startswith("remove_"):
                cart_remove_by_key(request, key[7:])
                messages.success(request, "Товар удалён из корзины.")
                return redirect("store:cart")
            if key.startswith("inc_"):
                line_key = key[4:]
                lines, _ = cart_lines(request)
                current = next((ln.quantity for ln in lines if ln.line_key == line_key), 0)
                cart_set_quantity_by_key(request, line_key, current + 1)
                return redirect("store:cart")
        ...
        for key, val in request.POST.items():
            if key.startswith("qty_"):
                ...
                try:
                    ...
                    continue
                cart_set_quantity_by_key(request, line_key, qty)
        messages.success(request, "Количества обновлены.")
        return redirect("store:cart")


class StorePaymentView(FormView):
    """Заглушка страницы оплаты (без реальной интеграции)."""

    template_name = "store/info/payment.html"
    form_class = PaymentStubForm
    ...
    def form_valid(self, form):
        messages.success(
            self.request,
            "Платёжные данные приняты (демо-заглушка, списание не выполняется). "
            "Перейдите к оформлению заказа.",
        )
        if self.request.user.is_authenticated:
            return redirect("store:checkout")
        return redirect(
            f"{reverse('store:login')}?next={reverse('store:checkout')}"
        )
...
class StoreReviewsView(StorePaginationQueryMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["show_review_form"] = self.request.user.is_authenticated and (
            self.request.GET.get("add") == "1" or review_form is not None
        )
    ...
    def post(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            next_url = reverse("store:reviews") + "?add=1"
            return redirect(f"{reverse('store:login')}?next={next_url}")

        profile = getattr(request.user, "customer_profile", None)
        if profile is None or profile.is_deleted:
            messages.error(request, "Оставлять отзывы могут только клиенты с профилем.")
            return redirect("store:reviews")

        form = StoreSiteReviewForm(request.POST)
        if not form.is_valid():
            self.object_list = self.get_queryset()
            return self.render_to_response(
                self.get_context_data(review_form=form, object_list=self.object_list)
            )
...
class StorePromosView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        today = date.today()
        all_promos = PromoCode.objects.filter(is_deleted=False).order_by("code")
        active_ids = set(
            PromoCode.objects.active_on(today).values_list("pk", flat=True)
        )
        ctx["active_promos"] = [p for p in all_promos if p.pk in active_ids]
        ctx["archived_promos"] = [p for p in all_promos if p.pk not in active_ids]
        return ctx
