...
class StoreSiteReviewForm(forms.ModelForm):
    """Форма отзыва со страницы «Отзывы» (с выбором товара)."""

    product = forms.ModelChoiceField(
        queryset=Product.objects.filter(is_deleted=False, is_active=True).order_by("name"),
        label="Товар",
    )
...
class PaymentStubForm(forms.Form):
    cardholder_name = forms.CharField(label="Имя держателя карты", max_length=120)
    card_number = forms.CharField(label="Номер карты", max_length=19, min_length=13)
