...
def cart_set_quantity_by_key(
    ...
    data = _cart_raw(request)
    if quantity < 1:
        data.pop(line_key, None)
    else:
        data[line_key] = min(quantity, 999)
    _save_cart(request, data)


def cart_remove_by_key(request: HttpRequest, line_key: str) -> None:
    data = _cart_raw(request)
    data.pop(line_key, None)
    _save_cart(request, data)
...
def cart_lines(request: HttpRequest) -> tuple[list[CartLineView], Decimal]:
    ...
    for line_key, pid, sid, qty in parsed:
